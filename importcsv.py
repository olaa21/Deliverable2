...
def generate_html(data, start_index, athlete_name):
    ...
    <head>
        ...
        <title>{html.escape(athlete_name)}</title>
    </head>
    <body>
        <h1>Performance Details for {html.escape(athlete_name)}</h1>
        <table>
            <thead>
                <tr>
                    <th>Name</th>
    ...
    for row in actual_data:
        ...
        overall_place = html.escape(row[1])
        grade = html.escape(row[2])
        time = html.escape(row[3])
        date = html.escape(row[4])
        meet = html.escape(row[5])
        comments = html.escape(row[6])
        photo = html.escape(row[7])

        # Ensuring 'photo' does not break the HTML structure
